<x-data-table.table  class="max-w-screen-xl" title="Students" itemType="student" :pagination="$students" :sortOptions="$sort_options" :filterOptions="$filter_options" minWidth="1280" :search="true">
    <x-slot name="actions">
    ...
    @if((count($students) == 0) && !$filtering)
        <x-data-table.data-row :selectable="false" bgClass="bg-white" id="">
            <td class="whitespace-nowrap" colspan="6">There are no students currently.</td>
        </x-data-table.data-row>
    @elseif((count($students) == 0) && $filtering)
        <x-data-table.data-row :selectable="false" bgClass="bg-white" id="">
            <td class="whitespace-nowrap" colspan="6">No students matched.</td>
            </x-data-table.data-row>
    @else
        <x-slot name="header">
            ...
            <td class="px-2 whitespace-nowrap text-right">Applications</td>
    ...
    <x-data-table.data-row :selectable="true" :bgClass="$i%2==0?'bg-white':'bg-gray-50'" :id="$user->id">
        ...
        <td class="px-2 whitespace-nowrap text-right">{{ \App\Models\Application::where(['student_user_id' => $user->id])->count() }}</td>
